...
static suite<"inline hook with no space"> inline_hook_tests = [] {
    "Test"_test = [&] {
        static SafetyHookInline hook0, hook1, hook2, hook3;
        struct Tests {
            static __declspec(noinline) void say_hi(const std::string& name, std::string& out) {
            ...
            static void reserve_memory_2gb_around_target(uintptr_t target) {
        ...
        Tests::reserve_memory_2gb_around_target(reinterpret_cast<uintptr_t>(&Tests::say_hi));

        std::cout << std::format("0x{:x}\n", (uintptr_t)&Tests::say_hi);

        uintptr_t real_say_hi = (uintptr_t)&Tests::say_hi;

        if (*(uint8_t*)&Tests::say_hi == 0xE9) {
            real_say_hi = (uintptr_t)&Tests::say_hi + *(int32_t*)((uintptr_t)&Tests::say_hi + 1) + 5;
    ...
    #error "Unsupported architecture"
        ...
        auto hook0_result = SafetyHookInline::create(reinterpret_cast<void*>(real_say_hi), reinterpret_cast<void*>(Tests::hook0_fn));
        expect(hook0_result.has_value());

        hook0 = std::move(*hook0_result);

        auto hook1_result = SafetyHookInline::create(reinterpret_cast<void*>(real_say_hi), reinterpret_cast<void*>(Tests::hook1_fn));
        expect(hook1_result.has_value());

        hook1 = std::move(*hook1_result);

        auto hook2_result = SafetyHookInline::create(reinterpret_cast<void*>(real_say_hi), reinterpret_cast<void*>(Tests::hook2_fn));
        expect(hook2_result.has_value());

        hook2 = std::move(*hook2_result);

        auto hook3_result = SafetyHookInline::create(reinterpret_cast<void*>(real_say_hi), reinterpret_cast<void*>(Tests::hook3_fn));
        ...
        if (*(uint8_t*)real_say_hi == 0xE9) {
            ...
            GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, reinterpret_cast<LPCSTR>(real_say_hi), &module_real_say_hi_in);
            GetModuleHandleExA(GET_MODULE_HANDLE_EX_FLAG_FROM_ADDRESS, reinterpret_cast<LPCSTR>(jmp_point), &module_jmp_point_in);
            
            const auto jmps_nearby = (mbi_real_say_hi.AllocationBase == mbi_jmp_point.AllocationBase &&
                mbi_real_say_hi.BaseAddress == mbi_jmp_point.BaseAddress) || (module_real_say_hi_in == module_jmp_point_in);
        ...
        std::string say_hi_storage{};
        Tests::say_hi("world", say_hi_storage);

        expect(say_hi_storage == "hello world and carol and eve and alice and bob");
    };
};